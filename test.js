...
const liste_best = [
  ...
  document.getElementById("best4"),
  document.getElementById("best5"),
  document.getElementById("best6"),
...
const liste_cat1 = [
  ...
  document.getElementById("com3"),
  document.getElementById("com4"),
  document.getElementById("com5"),
  document.getElementById("com6"),
  document.getElementById("com7"),
];

const url_hor =
  "http://localhost:8000/api/v1/titles/?genre=Horror&sort_by=-imdb_score";
const liste_cat2 = [
  document.getElementById("horror1"),
  document.getElementById("horror2"),
  document.getElementById("horror3"),
  document.getElementById("horror4"),
  document.getElementById("horror5"),
  document.getElementById("horror6"),
      ...
      if (resp.ok) {
        resp.json().then((data) => {
          fetch(data.results[0].url)
            .then((resp) => resp.json())
            .then((data) => {
              best_img.dataset.idmovie = data.id;
            ...
            .catch((error) => {
              console.log("probleme reponse fetch" + error.message);
            });
        });
      } else {
        console.log("probleme reponse url");
      }
    })
    .catch((error) => {
      ...
      if (resp.ok) {
        resp.json().then((data) => {
          for (let nb = 0; nb < 5; nb++) {
            ...
            liste_best[nb].dataset.idmovie = data.results[nb].id;
          }
          fetch(data.next)
            .then((resp) => {
              if (resp.ok) {
                resp.json().then((data) => {
                  for (let nb = 0; nb < 2; nb++) {
                    ...
                    liste_best[nb + 5].dataset.idmovie = data.results[nb].id;
